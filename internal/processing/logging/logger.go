...
// than mutating the receiver. A processing run logs from the cron goroutine, the
// manual-trigger goroutine and the poll loop, and a shared mutable
// context/transaction id would be both a data race and a source of mislabelled
// log lines.
type TLogger struct {
	layer         Layer
	context       string
...
// run_id ends up on every log line of a processing run.
